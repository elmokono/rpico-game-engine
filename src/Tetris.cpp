...
void Tetris::draw(void)
{
  ...
  if (currentStage == STAGE_INGAME || currentStage == STAGE_GAMEOVER)
  {
    ...
    engine_core_ref->spr_background.pushImage(0, 0, 128, 128, bgImage);

    // sprites
    for (int i = 0; i < BOARD_WIDTH; i++)
      for (int j = 0; j < (BOARD_HEIGHT - 1); j++)
        if (this->hasBlock(i, j))
          engine_core_ref->spr_background.pushImage(i * 8, j * 8, 8, 8, block_still); // engine_core_ref->canvas.drawRGBBitmap(i * 8, j * 8, block_still, 8, 8);

    if (currentStage == STAGE_INGAME)
    {
      Piece currentPiece = getCurrentPiece();
      for (int i = 0; i < BLOCKS_PER_PIECE; i++)
        engine_core_ref->spr_background.pushImage(
  ...
  if (currentStage == STAGE_GAMEOVER)
  {
    ...
    engine_core_ref->spr_background.pushImage(0, 48, 128, 32, gameover);
  }
}
